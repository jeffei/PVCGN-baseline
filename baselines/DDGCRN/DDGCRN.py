...
from baselines.DDGCRN.DDGCRNCell import DDGCRNCell
...
class DDGCRN(nn.Module):
    def __init__(self, args):
        ...
        self.node_embeddings1 = nn.Parameter(torch.randn(self.num_node, self.embed_dim), requires_grad=True)
        self.node_embeddings2 = nn.Parameter(torch.randn(self.num_node, self.embed_dim), requires_grad=True)
        self.T_i_D_emb = nn.Parameter(torch.empty(96, self.embed_dim))
    ...
    def forward(self, source, i=2):
        ...
        if i == 1:
            ...
            output1 = output1.reshape(-1, self.horizon, self.num_node, self.output_dim)    # B, T, N, 2

            source1 = self.end_conv2(output)

            source1 = source1.reshape(-1, self.horizon, self.num_node, self.output_dim)    # B, T, N, 2
            ...
            output2 = output2.reshape(-1, self.horizon, self.num_node, self.output_dim)    # B, T, N, 2
