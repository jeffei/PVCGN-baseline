...
from baselines.PM_DMnet.PM_Cell import PM_Cell
import numpy as np
...
class PM_DMNet(nn.Module):
    def __init__(self, args):
        ...
        self.T_i_D_emb = nn.Parameter(torch.empty(96, args.time_dim))
    ...
    def forward(self, source, traget=None, batches_seen=None):
        ...
        assert traget is not None
        steps_per_day = 96  # 288

        t_i_d_data1 = source[..., 0, -2]
        t_i_d_data2 = traget[..., 0, -2]
        # T_i_D_emb = self.T_i_D_emb[(t_i_d_data[:, -1, :] * 288).type(torch.LongTensor)]
        T_i_D_emb1 = self.T_i_D_emb[(t_i_d_data1 * steps_per_day).type(torch.LongTensor)]
        T_i_D_emb2 = self.T_i_D_emb[(t_i_d_data2 * steps_per_day).type(torch.LongTensor)]
        ...
        ## 改动点：
        node_embedding1 = T_i_D_emb1
        node_embedding2 = T_i_D_emb2
