...
def run_model(model, data_iterator, device, output_dim):
    """
...
def _get_log_dir(kwargs):
    ...
    if log_dir is None:
        ...
        num_rnn_layers = kwargs['model'].get('num_layers')
        ...
        K = kwargs['model'].get('cheb_order') or kwargs['model'].get('cheb_k')
        graph_type = kwargs['model'].get('graph_type', "DDGCRN")
...
def main(args):
    ...
    for epoch in range(cfg['train']['epochs']):
        ...
        for _, (x, y, xtime, ytime) in enumerate(train_iterator):
            ...
            label = scaler_torch.inverse_transform(label)
